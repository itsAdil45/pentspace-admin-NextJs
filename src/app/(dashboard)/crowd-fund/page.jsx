...
// import { crowdData } from '@/data/crowdfund';
import { crowdColumns } from '@/Columns/crowdCol';
import { API } from '@/api';
...
const totPage = Math.ceil((crowdData.length)/(pagination?.pageSize))
const [page, setPage]= useState(1)
  ...
  const handleCrowdFunding = async()=>{
    setLoader(true)
    try{
      const res = await API.getFunding()
          ...
          <TableBody>
            {loader ? (
              <TableRow className=" text-center">
                <TableCell colSpan={crowdColumns.length} className="h-24 ">
            ...
            ) : crowdData && table.getRowModel().rows?.length ? (
